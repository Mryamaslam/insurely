(function(){
  ...
  // cart helpers
  function getCart(){try{return JSON.parse(localStorage.getItem("insurely_cart")||"[]")}catch(e){return[]}}
  function setCart(c){localStorage.setItem("insurely_cart",JSON.stringify(c));updateCartCount()}
  ...
  if(checkoutForm)checkoutForm.addEventListener("submit",function(e){
    e.preventDefault();
    localStorage.removeItem("insurely_cart");
    updateCartCount();
